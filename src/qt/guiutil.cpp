...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("PinkBitz"))
        return false;
...
{
    // Convert PinkBitz:// to PinkBitz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("PinkBitz://"))
    {
        uri.replace(0, 12, "PinkBitz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PinkBitz.lnk";
}
...
{
    return GetAutostartDir() / "PinkBitz.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PinkBitz-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PinkBitz-qt [" + tr("command-line options") + "]                     " + "\n";
